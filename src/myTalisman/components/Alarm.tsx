...
import {
  timeFormater,
  transformAlarms,
} from "../helpers/timerDurationTransform";
...
export const Alarm = ({ sounds = [] }: TimerOptions) => {
  ...
  const handleAlarm = () => {
    if (
      ...
      (state.alarms[0].hours === null || state.alarms[0].minutes === null)
    ) {
      setWarning(true);
      toast.error("Existen alarmas sin definir", {
    ...
    if (
      ...
      (state.alarms[0].hours === null ||
        state.alarms[0].minutes === null ||
        state.alarms[1].hours === null ||
        state.alarms[1].minutes === null)
    ) {
      setWarning(true);
      toast.error("Existen alarmas sin definir", {
        style: { backgroundColor: "#6f3289", color: "#ece976" },
    ...
    if (
      ...
      (state.alarms[0].hours === null ||
        ...
        state.alarms[1].hours === null ||
        state.alarms[1].minutes === null ||
        state.alarms[2].hours === null ||
        state.alarms[2].minutes === null)
    ) {
      setWarning(true);
      toast.error("Existen alarmas sin definir", {
        style: { backgroundColor: "#6f3289", color: "#ece976" },
    ...
    if (
      ...
      (state.alarms[0].hours === null ||
        ...
        state.alarms[2].hours === null ||
        state.alarms[2].minutes === null ||
        state.alarms[3].hours === null ||
        state.alarms[3].minutes === null)
    ) {
      setWarning(true);
      toast.error("Existen alarmas sin definir", {
        style: { backgroundColor: "#6f3289", color: "#ece976" },
    ...
    const newAlarms = {
      alarm1:
        state.alarms[0].hours === null || state.alarms[0].minutes === null
          ...
          : `${
              state.alarms[0].hours !== null
                ? timeFormater(state.alarms[0].hours)
                : ""
            }:${timeFormater(state.alarms[0].minutes)}`,
      alarm2:
        state.alarms[1].hours === null ||
        state.alarms[1].minutes === null ||
        alarmMode === "1 vez al día"
          ...
          : `${
              state.alarms[1].hours !== null
                ? timeFormater(state.alarms[1].hours)
                : ""
            }:${timeFormater(state.alarms[1].minutes)}`,
      alarm3:
        state.alarms[2].hours === null ||
        state.alarms[2].minutes === null ||
        alarmMode === "1 vez al día" ||
        alarmMode === "2 veces al día"
          ...
          : `${
              state.alarms[2].hours !== null
                ? timeFormater(state.alarms[2].hours)
                : ""
            }:${timeFormater(state.alarms[2].minutes)}`,
      alarm4:
        state.alarms[3].hours === null ||
        state.alarms[3].minutes === null ||
        alarmMode === "1 vez al día" ||
        ...
        alarmMode === "3 veces al día"
          ...
          : `${
              state.alarms[3].hours !== null
                ? timeFormater(state.alarms[3].hours)
                : ""
            }:${timeFormater(state.alarms[3].minutes)}`,
    };
    ...
    axios
      ...
      .then(() => {
        toast.success("Alarma modificada", {
          style: { backgroundColor: "#6f3289", color: "#ece976" },
          hideProgressBar: true,
          autoClose: 4000,
        });
  ...
  return (
    <div className="alarm-container revealLogo">
      {timePicker && (
        <TimePicker
          hours={state.alarms[alarmIndex].hours}
          minutes={state.alarms[alarmIndex].minutes}
          nextHour={() => dispatch({ type: "NEXT_HOUR", index: alarmIndex })}
          previuosHour={() =>
            dispatch({ type: "PREVIOUS_HOUR", index: alarmIndex })
          }
          nextMinute={() =>
            dispatch({ type: "NEXT_MINUTE", index: alarmIndex })
          }
          previousMinute={() =>
            dispatch({ type: "PREVIOUS_MINUTE", index: alarmIndex })
          }
      ...
      {alarmMode !== "Nunca" && (
        <div className="alarm-inputs-container">
          <div
            ...
            className="alarmn-internal-input-conainer"
          >
            <p>Horario 1</p>{" "}
            <p>
              {timeFormater(state.alarms[0].hours)}:
      ...
      {warning && (
        <strong className="warning">EXISTEN ALARMAS SIN DEFINIR</strong>
      )}

      {loading ? (
        <BeatLoader color={"white"} speedMultiplier={0.4} />
      ) : (
        <Button text="Guardar alarma" onClick={handleAlarm} />
      )}
    </div>
  );
};
