...
export class BillingValidation {
  ...
  static create(object: {
    ...
    const { billingName, billingLastname, billingRfc, billingLegalName, billingTaxRegime } = object;
    ...
    if (billingRfc.length===13) {
     ...
     console.log("numbers",shouldBeNumbers)
      if(!regularExpresions.only_letters.test(shouldBeLetters) ||!regularExpresions.only_numbers.test(shouldBeNumbers) ){
        billingErrors.push({
          rfc: "RFC incorrecto",
        });
      }
