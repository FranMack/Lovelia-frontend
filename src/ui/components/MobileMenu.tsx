...
import { CloseIcon,LoginIcon,ShopingIcon,ContactUsIcon } from "../../assets/icons/icons"
import { useNavigate } from "react-router-dom"
import { FacebookIcon,InstagramIcon,TwitterIcon } from "../../assets/icons/icons";
import { MobileMenuContext } from "../../context";
import { useContext } from "react";

const navbarMenu=[{title:"Home",path:""},{title:"Talismán",path:"talisman-landing"},{title:"Meditaciones",path:"meditations"},{title:"Intenciones",path:"intenciones"},{title:"Tienda",path:"tienda"},{title:"Blog",path:"blog"}]

const icons=[{title:"Mi cuenta",icon:<LoginIcon/>,path:"profile"},
  ...
  return (
   ...
   <div key={item.title} className="icon-container" onClick={()=>linkTo(item.path)}>{item.icon}<small>{item.title}</small></div>
     ...
     {socialMedia.map((item,i)=>{
      return(
        <div key={i} className="icon-container" onClick={()=>{redirectTo(item.path)}}>
        {item.icon}
        </div>
        )
     })}
