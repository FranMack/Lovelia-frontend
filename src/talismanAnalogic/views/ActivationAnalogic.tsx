...
const restartActivation=()=>{
  ...
  return (
   <section className='activationAnalogic-container' style={{justifyContent:playing?"center":"start"}}>
    

   {(!playing && !activationFinished) &&<div className='activationAnalogic-text-container'>
    <h1>{`Intención: ${intention.toLocaleUpperCase()}`}</h1>
    {step===1 &&<><h2>Hola! te damos la bienvenida a la activación de tu talismán.</h2>
    <h3>Para activar tu talismán:</h3></>}
    <ol start={step}>
      {step===1 &&<li>Buscar un lugar tranquilo.</li>}
     {step===2 && <li>Haz tres respiraciones profundas.</li>}
     { step===3 && <><li>Con tu talismán en tus manos, repite 3 veces:<br/> 
      <strong>Yo "Nombre" decreto traer la abundancia en mi vida. </strong></li>
     <li>Reproduce la meditación a continuación,elegida para atraer abundancia.</li></>}
    </ol>
    </div>}

    {step<3 &&<Button text='Siguiente' onClick={handleStep}/>}

    {step===3 &&<div className={playing ? "activationAnalogic-image-container playing":"activationAnalogic-image-container"}>
      <img src={abundanciaImg} alt="intention" />
      <div className='icon-container' title='Play / Stop'>
        {playing ? <StopIcon onClick={stopActivation}/> : <PlayIcon onClick={playActivation}/>}
      </div>
      <div className='icon-container restart' title='Restart'>
        <RestartIcon onClick={restartActivation}/>
      </div>
    </div>}
    {activationFinished && <div className='activationAnalogic-text-container'>
    <h3>Cada vez que quieras recordar tu intención o reafirmarla puedes volver a escuchar esta meditación. </h3>
